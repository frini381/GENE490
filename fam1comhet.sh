...
#############################################################################################
    ...
    pat_file=paste0(dir,"/2559_2562_2560_2561_20230707_hg38_Split_ann_A0.01_HIGHMOD_DPGQ_CompHet1_out.tsv")
    ...
    pat_data_Split <- pat_data %>%  separate_rows(CSQ,sep=",") %>% tidyr::separate(col=CSQ, sep ='\\|',into=column_names, convert = TRUE, remove = FALSE, fill = "right") %>% select(-CSQ) %>% filter(SOURCE == "Ensembl") #separate each CSQ data element into each column and remove CSQ and ANN columns and filter so only Emsemble gene ids
     ...
     write.table(pat_data_Split, file=paste0(dir, "/R_Analysis/data/2559_2562_2560_2561_20230707_hg38_Split_ann_A0.01_HIGHMOD_DPGQ_CompHet1_DS_out.tsv"), quote=FALSE, sep='\t', row.names=F) #convert split paternal data into a tsv
...
mat_file=paste0(dir,"/2559_2562_2560_2561_20230707_hg38_Split_ann_A0.01_HIGHMOD_DPGQ_CompHet2_out.tsv") 
   ...
   mat_data_Split <- mat_data %>%  separate_rows(CSQ,sep=",") %>% tidyr::separate(col=CSQ, sep ='\\|',into=column_names, convert = TRUE, remove = FALSE, fill = "right") %>% select(-CSQ) %>% filter(SOURCE == "Ensembl")  #separate each CSQ data element into each column and remove CSQ and ANN columns and filter so only Emsemble gene ids
     ...
     write.table(mat_data_Split, file=paste0(dir, "/R_Analysis/data/2559_2562_2560_2561_20230707_hg38_Split_ann_A0.01_HIGHMOD_DPGQ_CompHet2_DS_out.tsv"), quote=FALSE, sep='\t', row.names=F) #convert split maternal data into a tsv
...
write.table(data_Split_gnomAD_SpliceAI, file=paste0(dir, "/R_Analysis/data/2559_2562_2560_2561_20230707_hg38_Split_ann_A0.01_HIGHMOD_DPGQ_AD_out.tsv"), quote=FALSE, sep='\t', row.names=F)
