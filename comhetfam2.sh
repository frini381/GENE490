...
#############################################################################################
    ...
    pat_data_Split <- pat_data %>%  separate_rows(CSQ,sep=",") %>% tidyr::separate(col=CSQ, sep ='\\|',into=column_names, convert = TRUE, remove = FALSE, fill = "right") %>% select(-ANN,-CSQ) %>% filter(SOURCE == "Ensembl") #separate each CSQ data element into each column and remove CSQ and ANN columns and filter so only Emsemble gene ids
...
mat_file=paste0(dir,"/4584_4585_4586_20230417_GRCh37_split_ann_AF0.01_HIGHMOD_CompHet2_out.tsv") 
   ...
   mat_data_Split <- mat_data %>%  separate_rows(CSQ,sep=",") %>% tidyr::separate(col=CSQ, sep ='\\|',into=column_names, convert = TRUE, remove = FALSE, fill = "right") %>% select(-ANN,-CSQ) %>% filter(SOURCE == "Ensembl")  #separate each CSQ data element into each column and remove CSQ and ANN columns and filter so only Emsemble gene ids
...
data_Split_comphetgenes <- rbind(mat_data_Split_comphetgenes,pat_data_Split_comphetgenes) %>% arrange(Gene) %>% filter(BIOTYPE=="protein_coding") %>% filter(IMPACT!="MODIFIER") ## filtering for protein coding and removing modifier variants and combining mat/pat variants into single object
...
write.table(data_Split_comphetgenes_gnomAD, file=paste0(dir, "/R_Analysis/data/4584_4585_4586_20230417_GRCh37_split_ann_AF0.01_HIGHMOD_CompHet_tog_out.tsv"), quote=FALSE, sep='\t', row.names=F)
